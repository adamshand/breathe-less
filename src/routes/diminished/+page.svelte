<script lang="ts">
	...
	onMount(() => {
		session.setExercise(diminishedExercise)
	})
	...
	$effect(() => {
		if (session.finished && session.sessionSaved) {
			if (window.umami)
				window.umami.track('Session finish', { exercise: 'diminished' })

			session.reset()
			goto('/history?exercise=diminished')
		} else {
			loadTodaysSessions()
		}
	})
...
<svelte:head>
	<title>Diminished Breathing | Breathe Less</title>
	<meta
		name="description"
		content="Practice the Diminished Breathing exercise - a gentler Buteyko variation."
	/>
</svelte:head>
